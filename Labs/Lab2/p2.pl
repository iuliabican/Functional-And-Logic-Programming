...
% flow model: merge_sort(i, o)is_list
...
% Mathematical model:
% split(l1...ln, c1...cm) =
%       return, abs(m - n) <= 1 -> either 1 or 0, otherwise it goes to
%              the recursive call
...
%split(Right, [], [], Right).
%split([H|T], [_,_|T1], [H|T2], Right) :-
%    split(T, T1, T2, Right).

% split(L:list, Left:list, Right:list)
% flow model: split(i, o, o)
%split(L, Left, Right) :- split(L, L, Left, Right).
...
% sort_list(L:list, R:list - result)
